...
public class AlertUtilsWeekly {
	...
	private void ValidationAlert() {
		...
		try {
			lstScannersAlert = ScannerAlertDao.init().listRecurrentsWeekly();
    ...
    private void filterDates(ScannerAlert alert) {
    	LocalDate referenceFirstDate = scanner.getResults().get(0).getQuery_date().toLocalDate().minusWeeks(1);
    	LocalDate referenceEndDate = scanner.getResults().get(0).getQuery_date().toLocalDate();
    ...
    private void matchingTrack(ScannerAlert alert) throws RemoveApplicationException {
    	...
    	LocalDate referenceFirstDate = scannerTrack.getResults().get(0).getQuery_date().toLocalDate().minusWeeks(1);
    	LocalDate referenceEndDate = scannerTrack.getResults().get(0).getQuery_date().toLocalDate();
    ...
    private void matchingKey(ScannerAlert alert) throws RemoveApplicationException {
    	...
    	LocalDate referenceFirstDate = scannerKey.getResults().get(0).getQuery_date().toLocalDate().minusWeeks(1);
    	LocalDate referenceEndDate = scannerKey.getResults().get(0).getQuery_date().toLocalDate();
	...
	private void pushKeywordByContent(List<GenericAlert> list, ScannerResult result,
		...
		try {
			ScannerKeyword keyword = ScannerKeywordsDao.init().get(result.getId_keyword());
			List<Long> listIdTW = ScannerResultNewsSnippetDao.init().loadTWIdsList(snippet.getId());
			List<ScannerTrackingWord> listTW = ScannerTrackingWordsDao.init().list(listIdTW);
			listTW =  listTW.stream().filter( x -> !x.getType().equalsIgnoreCase("URL") ).collect(Collectors.toList());
			GenericAlert generic = new GenericAlert();
			generic.setTitle(snippet.getTitle());
	...
	private void pushKeywordByContent(List<GenericAlert> list, ScannerResult result,
		...
		try {
			ScannerKeyword keyword = ScannerKeywordsDao.init().get(result.getId_keyword());
			List<Long> listIdTW = ScannerResultImageSnippetDao.init().loadTWIdsList(snippet.getId());
			List<ScannerTrackingWord> listTW = ScannerTrackingWordsDao.init().list(listIdTW);
			listTW =  listTW.stream().filter( x -> !x.getType().equalsIgnoreCase("URL") ).collect(Collectors.toList());
			GenericAlert generic = new GenericAlert();
			generic.setTitle(snippet.getTitle());
