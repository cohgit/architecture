...
public class ReportManagerFifteen extends TimerTask{
	private Logger log = LogManager.getLogger(ReportManagerFifteen.class);  
	protected RemoveBundle bundle;
	
	public ReportManagerFifteen() {
		super();
	}
	...
	public void run() {
		...
		log.info("Starting Report manager fifteen at:" + RemoveDateUtils.nowLocalDateTime());
		List<Client> lstclient = null;
		
		try {
			
			lstclient = ClientDao.init().listActivesFifteen();
